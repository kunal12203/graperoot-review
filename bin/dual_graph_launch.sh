...
RESUME_ID=""
PROMPT=""
FAILOVER_MODEL=""   # --model flag: codex | local | gemini | opencode

# Strip --model=X or --model X before the normal arg parser sees remaining args
_NEW_ARGS=()
for _a in "$@"; do
  if [[ "$_a" == --model=* ]]; then
    FAILOVER_MODEL="${_a#--model=}"
  elif [[ "$_a" == "--model" ]]; then
    _SKIP_NEXT=1
  elif [[ -n "${_SKIP_NEXT:-}" ]]; then
    FAILOVER_MODEL="$_a"
    unset _SKIP_NEXT
  else
    _NEW_ARGS+=("$_a")
...
# If --model flag given, override ASSISTANT immediately
if [[ -n "$FAILOVER_MODEL" ]]; then
  case "$FAILOVER_MODEL" in
    ...
    local|ollama)
      # Ollama via Codex with local endpoint — set env for codex to pick up
      ASSISTANT="codex"
      export OPENAI_BASE_URL="http://localhost:11434/v1"
      export OPENAI_API_KEY="ollama"
      # Auto-pick best available model
      _OLLAMA_MODEL="$(curl -sf --max-time 2 'http://localhost:11434/api/tags' 2>/dev/null \
      ...
      if [[ -n "$_OLLAMA_MODEL" ]]; then
        export OPENAI_DEFAULT_MODEL="$_OLLAMA_MODEL"
        echo "[$TOOL_LABEL] Using local model: $_OLLAMA_MODEL (via Ollama)"
      fi
      ;;
    *) echo "[$TOOL_LABEL] Unknown --model '$FAILOVER_MODEL'. Options: codex, local, gemini, opencode" >&2 ;;
  esac
fi
...
# GrapeRoot Pro: use AST-based index for 2.2× more symbols (+10 quality points on large repos)
export GRAPEROOT_AST=1
...
if [[ -f "\$AUDIT_CTX" ]]; then
  ...
  if [[ "\$_AGE_SEC" -lt 604800 ]]; then  # 7 days
    echo ""
    echo "=== Audit Context ==="
    cat "\$AUDIT_CTX"
...
if [[ "$ASSISTANT" == "claude" && "$ASSISTANT_EXIT" -ne 0 && "$ASSISTANT_EXIT" -ne 130 && "$ASSISTANT_EXIT" -ne 143 ]]; then
  _STDERR_CONTENT="$(cat "$DATA_DIR/assistant_stderr.log" 2>/dev/null || true)"
  if echo "$_STDERR_CONTENT" | grep -qiE "rate.limit|overloaded|too many request|quota exceeded|529|slowdown|capacity"; then
    ...
    if curl -sf --max-time 1 "http://localhost:11434/api/tags" >/dev/null 2>&1; then
      _HAS_FAILOVER=true
      _OLLAMA_MODELS="$(curl -sf --max-time 2 'http://localhost:11434/api/tags' 2>/dev/null \
        | python3 -c "import json,sys; ms=[m['name'] for m in json.load(sys.stdin).get('models',[])]; print(', '.join(ms[:3]))" 2>/dev/null || echo "local model")"
      echo "[$TOOL_LABEL]   Local (Ollama — $_OLLAMA_MODELS):"
      echo "[$TOOL_LABEL]     dgc --model=local \"$PROJECT\""
    fi

    if command -v gemini &>/dev/null; then
      _HAS_FAILOVER=true
      echo "[$TOOL_LABEL]   Gemini CLI:"
      echo "[$TOOL_LABEL]     dgc --model=gemini \"$PROJECT\""
    fi

    if [[ "$_HAS_FAILOVER" == "false" ]]; then
      echo "[$TOOL_LABEL]   No fallback model detected."
    ...
    echo ""
    echo "[$TOOL_LABEL]   Or wait for the Claude rate limit to reset and run dgc again."
    # Write failover suggestion to data dir so next launch can auto-suggest
    echo "claude_rate_limited_$(date +%s)" > "$DATA_DIR/.last_rate_limit" 2>/dev/null || true
  fi
fi
